...
def loadmodels(root, cuboid, suffix=""):
    ...
    def explore(path):
        ...
        if len(folders) > 0:
            ...
            # path_tex = os.path.join(path, "texture_map.png") # No disponemos de mapa de textura, por lo que se deja comentado

            if os.path.exists(path_obj): # and os.path.exists(path_tex): # no tenemos
                path = path.rstrip("/")
