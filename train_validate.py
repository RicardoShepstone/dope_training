...
# trainig data argument parser
parser = argparse.ArgumentParser()

# Specify Training Data
parser.add_argument("--train_data", nargs="+", help="Path to training data")
parser.add_argument("--test_data", nargs="+", help="Path to testing data")
parser.add_argument(
    "--use_s3", action="store_true", help="Use s3 buckets for training and testing data"
)
parser.add_argument(
    "--train_buckets",
    nargs="+",
...
parser.add_argument(
    ...
    nargs="+",
    default=[],
    help="s3 buckets containing testing data. Can list multiple buckets separated by a space.",
)
...
if opt.use_s3 and (opt.test_buckets is None or opt.endpoint is None):
    raise ValueError(
        "--test_buckets and --endpoint must be specified if testing with data from s3 bucket."
    )

if not opt.use_s3 and opt.train_data is None:
    raise ValueError("--train_data field must be specified.")


if not opt.use_s3 and opt.test_data is None:
    raise ValueError("--test_data field must be specified.")
...
def _trainnetwork(epoch, train_loader, syn=False):
...
def _testnetwork(epoch, test_loader):
    #net
    net.eval()

    test_loss_avg_to_log = {}
    test_loss_avg_to_log["loss"] = []
    test_loss_avg_to_log["loss_affinities"] = []
    test_loss_avg_to_log["loss_belief"] = []
    test_loss_avg_to_log["loss_class"] = []

    with torch.no_grad():
        for batch_idx, targets in enumerate(test_loader):
            data = Variable(targets["img"].cuda())
            target_belief = Variable(targets["beliefs"].cuda())
            target_affinities = Variable(targets["affinities"].cuda())
    ...
    # log the loss
    test_loss_avg_to_log["loss"].append(loss.item())
    test_loss_avg_to_log["loss_class"].append(loss_class.item())
    test_loss_avg_to_log["loss_affinities"].append(loss_affinities.item())
    test_loss_avg_to_log["loss_belief"].append(loss_belief.item())

    if batch_idx % opt.loginterval == 0:
        print(
            "Test Epoch: {} [{}/{} ({:.0f}%)] \tLoss: {:.15f} \tLocal Rank: {}".format(
                epoch,
                batch_idx * len(data),
    ...
    if local_rank == 0:
        ...
        writer.add_scalar("loss/test_loss", np.mean(test_loss_avg_to_log["loss"]), epoch)
        writer.add_scalar(
            "loss/test_cls", np.mean(test_loss_avg_to_log["loss_class"]), epoch
        )
        writer.add_scalar(
            "loss/test_aff", np.mean(test_loss_avg_to_log["loss_affinities"]), epoch
        )
        writer.add_scalar(
            "loss/test_bel", np.mean(test_loss_avg_to_log["loss_belief"]), epoch
        )


for epoch in range(1, opt.epochs + 1):

    _trainnetwork(epoch, trainingdata)
    ...
    _testnetwork(epoch, testingdata)
